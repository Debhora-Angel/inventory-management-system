...
class User:
    ...
    def user_input(self):
       ...
       entercart = ttk.Combobox(self.entryframe, width=20, textvariable=self.additem, font="roboto 12")
       ...
       entercart['values'] = self.inventory
    ...
    def addtotrans(self):
        if(len(self.additem.get()) == 0 or self.inventory.count(self.additem.get())==0):
            ...
            if(int(self.qty.get()) <= 0):
                messagebox.showerror('Error', 'Invalid quantity!')
                return
            self.cur.execute("select product_id,product_name from products where product_name = ? ", (self.additem.get(),))
